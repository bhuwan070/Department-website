import { AcesExcessMsgs } from '../../data/data';

const AcesExcessMsg = () => {
  const messages = AcesExcessMsgs;
  return (
    <div
      className={` mx-auto w-full h-fit grid grid-cols-1 lg:grid-cols-2 gap-4 font-[Montserrat] text-white`}
    >
      {messages?.map((message, index) => (
        ...
        >
          ...
          <div className="w-full h-full grid gap-2 lg:gap-4  ">
            ...
            <p
              className={`font-[Montserat,sans] text-gray-600 text-[14px] md:text-[16px] max:h-[24rem] min-h-fit leading-4 md:leading-5 `}
            >
              {message.content}
            </p>
          </div>
